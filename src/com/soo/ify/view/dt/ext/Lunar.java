 ...
 */
package com.soo.ify.view.dt.ext;
...
/**Thank OSC（website[http://www.oschina.net/code/snippet_216465_38919?p=2#comments]）
 * @author Soo
 */
public class Lunar {
    /**
    ...
    public String toString() {
        ...
        if (Utils.isNullOrEmpty(message)) {
            String solarMsg = new SolarTermsUtil(mCurrenCalendar)
                    .getSolartermsMsg();
            // 判断当前日期是否为节气
            if (!Utils.isNullOrEmpty(solarMsg)) {
                message = solarMsg;
            } else {
                /**
                 * 判断当前日期是否为公历节日
                ...
                String gremessage = new GregorianUtil(mCurrenCalendar)
                        .getGremessage();
                if (!Utils.isNullOrEmpty(gremessage)) {
                    message = gremessage;
                } else if (mLuchDay == 1) {
    ...
    public static String getCurrentDay(Calendar calendar) {
        return getDay(calendar) + " 农历" + new Lunar(calendar).getDay()
                + " " + getWeek(calendar);
    }
